...
# Core tasks for FLC ablation comparison.
FLC_TASKS = [
    "self_state_meaning",
    "delayed_lure",
    "triadic_binding",
...
# Optional training terms. Defaults preserve the base training path.
FEATURE_FLAGS = {
    # Counterfactual loss for FiLM self-state contrast pairs.
    ...
    *,
    ...
    if training_signal == "terminal_homeostasis":
        ...
        if next_self_state is None:
            raise ValueError("terminal_homeostasis requires next self_state")
    ...
    if self_state is None or next_self_state is None:
        raise ValueError("homeostatic_delta requires current and next self_state")
    current = np.asarray(self_state, dtype=np.float32)
    following = np.asarray(next_self_state, dtype=np.float32)
    current_viability = float(np.clip(1.0 - current.sum(), -1.0, 1.0))
    next_viability = -1.0 if was_lethal else float(np.clip(1.0 - following.sum(), -1.0, 1.0))
...
def _tensor_health(tensor):
    ...
    with torch.no_grad():
        ...
        return {
            ...
            "norm": (
                float(torch.linalg.norm(finite_values).item()) if finite_values.numel() else 0.0
            ),
            "max_abs": (float(finite_values.abs().max().item()) if finite_values.numel() else 0.0),
        }
...
def _passive_boundary_target(obs):
    ...
    if obs is None or len(obs) < 4:
        return 0.0
    boundary_pressure = float(np.max(np.asarray(obs[2:4], dtype=np.float32)))
    if boundary_pressure <= 0.6:
        return 0.0
    if boundary_pressure <= 0.8:
        return float(0.3 + 0.4 * ((boundary_pressure - 0.6) / 0.2))
...
def _compute_counterfactual_loss(
    ...
    """empirical hinge-based counterfactual loss for FiLM self-state.
    ...
    pairs. The consequence "value" prediction must differ by at
    ...
    # Object features (positions start at index 6) and context (4:6)
...
def build_training_param_groups(agent, lr, profile="legacy"):
    ...
    flc_param_ids = {id(p) for p in agent.flc.parameters()} if hasattr(agent, "flc") else set()
    grounded_inverse_param_ids = (
        ...
        if hasattr(agent, "flc") and hasattr(agent.flc, "grounded_transfer")
        else set()
    )
    ...
    if grounded_inverse_params:
        ...
        groups.append({"params": grounded_inverse_params, "lr": lr * other_multiplier})
    if flc_params:
...
def _observe_trajectory_step(
    ...
    *,
    ...
    following_body,
    terminal_boundary,
    situated_entities=None,
    event_mask=None,
):
    """Dispatch a detached, post-recoupling trace to a passive observer."""
    if hasattr(observer, "observe_event_consequence"):
        if situated_entities is None or event_mask is None:
            raise ValueError("event observers require situated entities and event mask")
        observer.observe_event_consequence(
            current_latent,
            released_action,
            recoupled_latent,
            current_body,
    ...
    elif hasattr(observer, "observe_body_consequence"):
        observer.observe_body_consequence(
            current_latent,
            released_action,
            recoupled_latent,
            current_body,
    ...
    elif hasattr(observer, "observe_action_consequence"):
        observer.observe_action_consequence(
            current_latent,
            released_action,
            recoupled_latent,
        )
    else:
        observer.observe(current_latent, recoupled_latent)
...
def train_agent(
    ...
    if credit_mode not in CREDIT_MODES:
        raise ValueError(f"unknown credit_mode={credit_mode!r}; expected one of {CREDIT_MODES}")
    if optimizer_profile not in OPTIMIZER_PROFILES:
        raise ValueError(
            f"unknown optimizer_profile={optimizer_profile!r}; expected one of {OPTIMIZER_PROFILES}"
        )
    ...
    # the policy output. This remains constitutional because all components learn
    # end-to-end, but at appropriate rates.
    optimizer = optim.Adam(build_training_param_groups(agent, lr, profile=optimizer_profile))
    bpsi_enabled = (
        hasattr(agent, "veto")
    ...
    bpsi_optimizer = optim.Adam(agent.veto.parameters(), lr=1e-3) if bpsi_enabled else None
    if bpsi_enabled:
    ...
    # subtracting it would zero out the gradient. A sliding window of recent
    # episode returns supplies the baseline. This is a
    # a standard REINFORCE-with-baseline technique (variance reduction, NOT
    # reward shaping — rewards remain unchanged; only the policy gradient is centered around the
    ...
    for episode in range(num_episodes):
        ...
        if trajectory_observer is not None:
            trajectory_observer.begin_episode()
        ...
        # self-supervised boundary targets for B_psi.
        ...
        while not done:
            ...
            if policy_log_prob is None:
                raise RuntimeError("training action did not expose its rollout log-probability")
            episode_policy_log_probs.append(policy_log_prob)
            intent_log_prob = info_dict.get("_intent_log_prob")
            if intent_log_prob is not None:
                episode_intent_scores.append((len(episode_policy_log_probs) - 1, intent_log_prob))
            if not hasattr(agent, "net"):
                ...
                episode_training_abstract_actions.append(info_dict["_training_abstract_action"])
            ...
            episode_homeostatic_deltas.append((float(hunger_delta), float(fatigue_delta)))

            # boundary proximity provides a continuous signal before
            # actual death. The paired zero-action target teaches B_psi the
            # baseline risk of inaction at the same body state.
            episode_boundary_targets.append(_boundary_proximity_target(obs, next_obs, was_lethal))
            ...
            if trajectory_observer is not None:
                current_snapshot = info_dict["_training_h"].detach().clone()
                recoupled_snapshot = agent.h.detach().clone()
                _observe_trajectory_step(
                    trajectory_observer,
                    current_latent=current_snapshot,
                    released_action=torch.as_tensor(action).detach().clone(),
                    recoupled_latent=recoupled_snapshot,
                    current_body=torch.as_tensor(obs[2:4]).detach().clone(),
                    following_body=torch.as_tensor(next_obs[2:4]).detach().clone(),
                    terminal_boundary=torch.as_tensor(was_lethal).detach().clone(),
                    situated_entities=(
                        info_dict["_training_sigma"].detach().clone()
                        if "_training_sigma" in info_dict
                        else None
                    ),
                    event_mask=(
                        agent.last_entity_event_mask.detach().clone()
                        if hasattr(agent, "last_entity_event_mask")
                        else None
                    ),
                )
            if not hasattr(agent, "net"):
                episode_entity_event_masks.append(agent.last_entity_event_mask.detach().clone())
                episode_observed_effects.append(agent.last_observed_effect.detach().clone())
                episode_post_latents.append(agent.h.detach().clone())

            obs = next_obs

        if trajectory_observer is not None:
            trajectory_observer.end_episode()

        # 1. Compute return G_t for policy gradient calculation
        returns = []
        G = 0.0
        return_gamma = (
            1.0 if training_signal in ("homeostatic_delta", "terminal_homeostasis") else 0.95
        )
        ...
        running_mean = sum(_return_history) / len(_return_history) if _return_history else 0.0
        ...
        # blend in RTCM delayed-credit blame (top-k softmax retrieval).
        # The delayed-credit weights point at the top-k past steps most likely
        # to have caused the observed consequence. A 50/50 blend combines this with the
        ...
        if (
            credit_mode == "rtcm"
            and hasattr(agent, "rtcm")
            and not (hasattr(agent, "ablation_type") and agent.ablation_type == "no_rtcm")
        ):
            delayed = agent.rtcm.retrieve_delayed_credit(consequence_vec, agent.h, top_k=3)
            if delayed and len(delayed) == len(episode_obs):
                ...
                for idx, w in delayed:
                    if 0 <= idx < len(delayed_full):
                        delayed_full[idx] += w
                # The result is a per-step scaling; the
                ...
                if len(blame_weights) == len(delayed_full):
                    blame_weights = [
                        0.5 * b + 0.5 * d for b, d in zip(blame_weights, delayed_full, strict=False)
                    ]
        blame_weights = _mean_one_credit_weights(blame_weights, len(episode_obs))
        ...
        policy_loss = _score_function_policy_loss(
            episode_policy_log_probs,
            advantages,
            blame_weights=(blame_weights if not hasattr(agent, "net") else None),
        ...
        if not hasattr(agent, "net"):
            ...
            for idx, sigma_t in enumerate(episode_training_sigmas):
                ...
                if bool(event_mask.any()):
                    was_lethal_step = float(episode_infos[idx]["status"] in ("death", "starvation"))
                    ...
                    target_risk = torch.tensor([was_lethal_step], device=device)
                    hunger_delta, fatigue_delta = episode_homeostatic_deltas[idx]
                    target_reversibility = torch.tensor(
                        [float(not was_lethal_step and hunger_delta + fatigue_delta < 0.0)],
                        device=device,
                    )
                    for ent_idx in torch.nonzero(event_mask, as_tuple=False).flatten().tolist():
                        ...
                        pred_reversibility = consequences_pred["reversibility"][0, ent_idx]
                        pred_uncertainty = consequences_pred["uncertainty"][0, ent_idx]
                        effect_loss = nn.functional.mse_loss(pred_effect, target_effect)
                        value_loss = nn.functional.mse_loss(pred_value, target_value)
                        risk_loss = nn.functional.binary_cross_entropy(pred_risk, target_risk)
                        ...
                        with torch.no_grad():
                            uncertainty_target = torch.clamp(
                                effect_loss.detach() + value_loss.detach() + risk_loss.detach(),
                                0.0,
                                1.0,
                ...
                if bpsi_enabled:
                    ...
                    action_weight = (
                        12.0 if was_lethal_step else (4.0 if attributable_risk > 0.05 else 1.0)
                    )
        ...
        if not hasattr(agent, "net") and getattr(agent, "use_prospective_event_grounding", False):
            max_horizon = agent.prospective_event_field.max_horizon
            for event_idx, event_mask in enumerate(episode_entity_event_masks):
                ...
                event_loss = agent.prospective_event_field.event_loss(
                    ...
                    actions=torch.cat(
                        episode_training_abstract_actions[start_idx : event_idx + 1],
                        dim=0,
                    ),
                    effects=torch.cat(
                    ...
                    lethal=float(episode_infos[event_idx]["status"] in ("death", "starvation")),
                )
                if event_loss is not None:
                    ...
                    endpoint = episode_post_latents[event_idx]
                    inverse_weights = agent.prospective_event_field.eligibility_weights(
                        event_latents,
                        event_effects,
                        endpoint,
                    ).detach()
                    ...
                    event_self_state = torch.as_tensor(
                        np.stack(episode_self_states[start_idx : event_idx + 1]),
                        dtype=torch.float32,
                        device=device,
                    )
                    prospective_inverse_losses.append(
                        agent.flc.grounded_inverse_loss(
                            ...
                            sigma_flat=event_sigmas.reshape(event_sigmas.shape[0], -1),
                    ...
                    event_entities = torch.nonzero(event_mask, as_tuple=False).flatten().tolist()
                    event_risk = float(
                        episode_infos[event_idx]["status"] in ("death", "starvation")
                    )
                    for offset in sampled_offsets:
                        ...
                        for entity_index in event_entities:
                            agent.prospective_event_memory.add(
                                ...
                                risk=event_risk,
                                action=episode_training_abstract_actions[trace_idx],
                                horizon=event_idx - trace_idx + 1,
                            )

        # Empirical counterfactual loss for FiLM self-state modulation.
        ...
        if FEATURE_FLAGS["counterfactual_loss"] > 0.0:
            ...
            if cf_loss is not None and cf_loss.requires_grad:
                total_loss = total_loss + FEATURE_FLAGS["counterfactual_loss"] * cf_loss
        ...
        # B_psi is NO LONGER trained in the main loss.
        ...
        if prospective_event_losses:
            total_loss = total_loss + torch.stack(prospective_event_losses).mean()
        if prospective_inverse_losses:
            total_loss = total_loss + torch.stack(prospective_inverse_losses).mean()
        ...
        # Dedicated B_psi boundary-risk training step.
        ...
        if bpsi_optimizer is not None and episode_bpsi_data:
            ...
            targets = [
                float(target_t.detach().item()) for _, _, _, target_t, _ in episode_bpsi_data
            ]
            weights = [
                float(weight_t.detach().item()) for _, _, _, _, weight_t in episode_bpsi_data
            ]
            agent._bpsi_training_stats.append(
                {
                    ...
                    "attributable_nonzero_rate": float(np.mean([t > 0.0 for t in targets]))
                    if targets
                    else 0.0,
        ...
        if hasattr(agent, "rtcm") and not (
            hasattr(agent, "ablation_type") and agent.ablation_type in ("no_rtcm", "no_memory")
        ):
        ...
        # Log episode metrics for learning curves
        final_info = episode_infos[-1]
        ...
        # Periodic self-state swap probe.
...
def evaluate_agent(agent, task_name, num_episodes=50, seed=None, training_signal=None):
    """
    Evaluates agent on the specified task and logs metrics.
    """
    set_global_seed(seed)
    agent.eval()
    signal_mode = training_signal or getattr(agent, "_training_signal", "legacy_reward")
    EnvClass = ENV_MAP[task_name]
    env = EnvClass(seed=seed)
...
def run_task_experiment(task, ablations):
    ...
    for seed in seeds:
        ...
        for abl_type in ablations:
            ...
            abl_agent = train_agent(
                abl_agent, task, num_episodes=300, seed=seed, agent_type=f"abl_{abl_type}"
            )
    ...
    return task, {"OLF": olf_stats, "MLP": mlp_stats, "Ablations": ablation_results}


def _evaluate_with_diagnostics(agent, task_name, seed, num_episodes=20, training_signal=None):
        ...
        for condition, ablation_type in [
            ...
            for seed in seeds:
                ...
                if ablation_type is None:
                    ...
                    agent = AblatedOrganism(obs_dim=18, action_dim=3, ablation_type=ablation_type)
                ...
                deaths = sum(1 for ep in eval_episodes if ep["status"] in ("death", "starvation"))
...
def main():
    ...
    experiments = [
        ...
        (
            "abstraction_unseen",
            ["no_consequence_memory", "exact_episodic_memory_only", "no_abstraction"],
        ),
        ("abstraction_unseen_randompos", []),  # leakage diagnostic
        ("negative_control", []),
        ("randomized_consequence", []),
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                print(
                    f"  [Full OLF] Success: {result['OLF']['success_rate']:.1%}, Safety: {result['OLF']['safety_rate']:.1%}"
                )
                print(
                    f"  [MLP Baseline] Success: {result['MLP']['success_rate']:.1%}, Safety: {result['MLP']['safety_rate']:.1%}"
                )
                for abl_type, stats in result["Ablations"].items():
                    print(
                        f"  [{abl_type}] Success: {stats['success_rate']:.1%}, Safety: {stats['safety_rate']:.1%}"
                    )
    ...
    print("\n" + "=" * 115)
    print(
        f"{'Task':<25} | {'OLF success':<18} | {'NFL success':<18} | {'Δ success':<12} | {'Δ danger':<12} | {'Δ rollback':<12}"
    )
    print("-" * 115)
    for task in FLC_TASKS:

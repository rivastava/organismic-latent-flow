...
class SituatedGapEnv(BaseBenchmark):
    """
    10. Situated affordance-gap task.
    Under impasse, the correct u depends on Object + Context:
    - Door Type 0 (context=[1,0]): u > 0.5 is correct.
    - Door Type 1 (context=[0,1]): u < -0.5 is correct.
    """
    def setup_task(self):
        # Select type
        self.door_type = self.rng.choice([0, 1])
        
        # Door (Entity 0)
        self.entities[0]["pos"] = np.array([0.0, 0.3], dtype=np.float32)
        if self.door_type == 0:
            self.entities[0]["feats"] = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)
            self.context = np.array([1.0, 0.0], dtype=np.float32)
        else:
            self.entities[0]["feats"] = np.array([0.0, 1.0, 0.0, 0.0], dtype=np.float32)
    ...
    def step(self, action):
        ...
        if target_y > 0.25 and not self.door_opened:
            if (self.door_type == 0 and u > 0.5) or (self.door_type == 1 and u < -0.5):
